...
class PyKernelDecorator(object):
    ...
    def compiledModuleCache(self):
        """Return this kernel's shared compiled-module cache.

        Create the cache lazily, but retain one instance for the decorator so
        every synchronous launch — and later every outstanding asynchronous
        launch — coordinates through the same compilation state.
        """
